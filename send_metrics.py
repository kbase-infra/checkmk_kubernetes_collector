...
def machine_sections_worker(
    ...
    command = ["/usr/local/bin/check_mk_agent"]
    ...
    try:
        ...
        if process.returncode != 0:
            logger.error(f"Agent execution failed with error: {stderr.strip()}")
            raise RuntimeError("Agent execution failed.")

        sections = stdout.strip()
        logger.debug(f"Received sections: {sections}")
